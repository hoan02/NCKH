...
public class BigRock extends Entity {
  ...
  public void detectPlate() {
    ...
    for (int i = 0; i < gp.npc[1].length; i++) {
      if (gp.npc[gp.currentMap][i] != null && gp.npc[gp.currentMap][i].name.equals(BigRock.npcName)) {
    ...
    if (count == rockList.size()) {
      for (int i = 0; i < gp.obj[1].length; i++) {
        if (gp.obj[gp.currentMap][i] != null && gp.obj[gp.currentMap][i].name.equals(DoorIron.objName)) {
