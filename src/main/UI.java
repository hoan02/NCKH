...
public class UI {
  ...
  public UI(GamePanel gp) {
    this.gp = gp;
    try {
      InputStream is = getClass().getClassLoader().getResourceAsStream("res/font/x12y16pxMaruMonica.ttf");
      maruMonica = Font.createFont(Font.TRUETYPE_FONT, is);
      is = getClass().getClassLoader().getResourceAsStream("res/font/Purisa Bold.ttf");
  ...
  public void drawDialogueScreen() {
    ...
    if (npc.dialogues[npc.dialogueSet][npc.dialogueIndex] != null) {
      // currentDialogue = npc.dialogues[npc.dialogueSet][npc.dialogueIndex];//For
      // display text once, enable this and disable letter by letter.(Letter by
      // letter: The if statement below there)
  ...
  public void trade_buy() {
    ...
    if (itemIndex < npc.inventory.size()) {
      ...
      if (gp.keyH.enterPressed == true) {
        ...
        // else if(gp.player.inventory.size() == gp.player.maxInventorySize) //full
        // inventory
        // {
        // subState = 0;
        ...
        // {
        // gp.player.coin -= npc.inventory.get(itemIndex).price; //-price
        // gp.player.inventory.add(npc.inventory.get(itemIndex)); //add item to player's
        // inventory
        // }
      }
    }
    ...
    for (int i = 0; i < gp.monster[1].length; i++) {
      ...
      if (monster != null && monster.inCamera() == true) {
        if (monster.hpBarOn == true && monster.boss == false) {
          double oneScale = (double) gp.tileSize / monster.maxLife; // (bar lenght / maxlife) Ex: if monster hp = 2,
                                                                    // tilesize = 48px. So, 1 hp = 24px
          ...
          double oneScale = (double) gp.tileSize * 8 / monster.maxLife; // (bar lenght / maxlife) Ex: if monster hp = 2,
                                                                        // tilesize = 48px. So, 1 hp = 24px
          double hpBarValue = oneScale * monster.life;
